      ...
      if (player) {
        try {
          if (action === 'play') {
            const p = player.play();
            if (p && p.catch) {
              p.catch(err => Logger.error('[Evo ve Beko Inject] Netflix play promise error:', err));
            }
          } else if (action === 'pause') {
            player.pause();
          } else if (action === 'seek') {
      ...
      if (player) {
        try {
          if (action === 'play') {
            const p = player.play();
            if (p && p.catch) {
              p.catch(err => Logger.error('[Evo ve Beko Inject] Disney+ play promise error:', err));
            }
          } else if (action === 'pause') {
            player.pause();
          } else if (action === 'seek') {
      ...
      if (player) {
        try {
          if (action === 'play' && typeof player.playVideo === 'function') {
            ...
            if (p && p.catch) {
              p.catch(err => Logger.error('[Evo ve Beko Inject] YouTube play promise error:', err));
            }
