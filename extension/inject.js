    ...
    if (isNetflix) {
      ...
      if (player) {
        try {
          if (action === 'play') {
            const p = player.play();
            if (p && p.catch) p.catch(err => Logger.error('[Evo ve Beko Inject] Netflix play hatası:', err));
          } else if (action === 'pause') {
            player.pause();
          } else if (action === 'seek') {
    ...
    // Disney+ Kontrolleri
      ...
      if (player) {
        try {
          if (action === 'play') {
            const p = player.play();
            if (p && p.catch) p.catch(err => Logger.error('[Evo ve Beko Inject] Disney+ play hatası:', err));
          } else if (action === 'pause') {
            player.pause();
          } else if (action === 'seek') {
      ...
      if (player) {
        try {
          if (action === 'play' && typeof player.playVideo === 'function') {
            ...
            if (p && p.catch) p.catch(err => Logger.error('[Evo ve Beko Inject] YouTube play hatası:', err));
