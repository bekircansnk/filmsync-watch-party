        ...
        try {
          if (action === 'play') {
            ...
            if (playPromise !== undefined) {
              playPromise.catch(err => Logger.error("[Evo ve Beko Inject] Play error:", err));
            }
          } else if (action === 'pause') {
            player.pause();
          } else if (action === 'seek') {
      ...
      if (player) {
        try {
          if (action === 'play' && typeof player.playVideo === 'function') {
            try { player.playVideo(); } catch (e) { Logger.error("[Evo ve Beko Inject] YouTube Play error:", e); }
          } else if (action === 'pause' && typeof player.pauseVideo === 'function') {
            try { player.pauseVideo(); } catch (e) { Logger.error("[Evo ve Beko Inject] YouTube Pause error:", e); }
          } else if (action === 'seek' && typeof player.seekTo === 'function') {
            try { player.seekTo(value, true); } catch (e) { Logger.error("[Evo ve Beko Inject] YouTube Seek error:", e); } // YouTube API saniye cinsinden seek alır
          }
        } catch (err) {
