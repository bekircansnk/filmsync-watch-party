...
const PlayerAdapter = {
  ...
  play: () => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      const p = videoElement.play(); if (p && p.catch) p.catch(err => Logger.error('[FilmSync] Play hatası:', err));
    }
  },
  ...
  ensureVideoReady((isReady) => {
    ...
    try {
      ...
      if (state.isPlaying && videoElement.paused) {
        ...
        const p = PlayerAdapter.play(); if (p && p.catch) p.catch(err => Logger.error('[FilmSync] Play hatası:', err));
