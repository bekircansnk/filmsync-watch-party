...
function startDriftCorrection() {
  setInterval(() => {
    ...
    db.ref(`rooms/${roomId}/lastState`).once('value').then((snapshot) => {
      ...
      // Dinamik tolerans hesaplama (Ağ gecikmesi ve durum uyumsuzluğuna göre)
      const playStateMismatch = state.isPlaying !== !videoElement.paused;

      // Durum uyuşmazlığı varsa daha düşük, yoksa gecikme(timeDiff) bazlı dinamik eşik (min 2.0s, max 4.0s)
      const dynamicThreshold = playStateMismatch
        ? 1.0
        : Math.min(4.0, 2.0 + (timeDiff * 0.5));
