...
const PlayerAdapter = {
  ...
  play: () => {
    try {
      if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
        ...
        videoElement.play().catch(e => Logger.error('[FilmSync] Playback error:', e));
      }
    } catch (e) {
      Logger.error('[FilmSync] PlayerAdapter.play error:', e);
    }
  },
  ...
  pause: () => {
    try {
      ...
      Logger.error('[FilmSync] PlayerAdapter.pause error:', e);
    }
  },
  ...
  seek: (seconds) => {
    try {
      ...
      Logger.error('[FilmSync] PlayerAdapter.seek error:', e);
    }
  }
};
