...
const PlayerAdapter = {
    ...
    try {
      if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(e => {
            Logger.error('[FilmSync] Play promise rejected:', e);
          });
        }
      }
    } catch (e) {
      Logger.error('[FilmSync] Error in PlayerAdapter.play:', e);
    }
  },
  ...
  pause: () => {
    try {
      ...
      Logger.error('[FilmSync] Error in PlayerAdapter.pause:', e);
    }
  },
  ...
  seek: (seconds) => {
    try {
      ...
      Logger.error('[FilmSync] Error in PlayerAdapter.seek:', e);
    }
  }
};
