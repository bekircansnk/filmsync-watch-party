...
function toggleChatPanel() {
  ...
  if (isOpened) {
    ...
    if (chatToggleBtn) {
      ...
      chatToggleBtn.appendChild(safeHTML('<svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>'));
    }
  } else {
    document.body.classList.remove('filmsync-sidebar-open');
    ...
    if (chatToggleBtn) {
      ...
      chatToggleBtn.textContent = '';
      chatToggleBtn.appendChild(safeHTML('<svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 9h12v2H6V9zm8 5H6v-2h8v2zm4-6H6V6h12v2z"/></svg>'));
    }
  }
...
function showNamePromptModal(roomName, callback) {
  ...
  titleDiv.appendChild(safeHTML('FilmSync <span>Partisi</span> 🍿'));
  ...
  button.id = 'promptJoinBtn';
  button.setAttribute('style', 'width: 100%; padding: 11px; border: none; border-radius: 8px; background: linear-gradient(135deg, #45f3ff, #66fcf1); color: #0b0c10; font-size: 0.85rem; font-weight: 700; cursor: pointer; transition: transform 0.2s;');
  button.textContent = 'Odaya Katıl';
...
function injectNetflixStartButton() {
  ...
  buttonContainers.forEach(container => {
    ...
    startBtn.setAttribute('style', `
      ...
      font-family: inherit;
    `);
    startBtn.appendChild(safeHTML('<span>Evo & Beko Partisi Başlat</span> 🍿'));
