...
function showAutoJoinOverlay(roomName) {
  ...
  `;
  const textContainer = overlay.querySelector('#filmsync-autojoin-text');
  if (textContainer) {
    textContainer.textContent = `"${roomName}" Odasına Katılınıyor...`;
  }
...
function showNamePromptModal(roomName, callback) {
  ...
  const textContainer = modal.querySelector('#filmsync-name-prompt-text');
  if (textContainer) {
    textContainer.textContent = `"${roomName}" odasına katılacaksınız.`;
  }
