...
function startDriftCorrection() {
  setInterval(() => {
    ...
    db.ref(`rooms/${roomId}/lastState`).once('value').then((snapshot) => {
      ...
      // Oynatma durumu uyuşmuyorsa veya süre sapması dinamik eşiği aşıyorsa otomatik eşitle
      const playStateMismatch = state.isPlaying !== !videoElement.paused;

      const dynamicThreshold = playStateMismatch ? 0.5 : Math.max(1.5, Math.min(timeDiff * 1.5, 4.0));
