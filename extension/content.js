...
const PlayerAdapter = {
  ...
  lockEvents: false,
  _eventLockTimeout: null,

  lock: (duration, callback) => {
    PlayerAdapter.lockEvents = true;
    if (PlayerAdapter._eventLockTimeout) {
      clearTimeout(PlayerAdapter._eventLockTimeout);
    }
    PlayerAdapter._eventLockTimeout = setTimeout(() => {
      ...
      if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(error => {
            console.warn('[FilmSync] Play promise rejected:', error);
          });
        }
      }
    } catch (e) {
      console.error('[FilmSync] Error in play:', e);
    }
  },
  ...
  pause: () => {
    try {
      ...
      console.error('[FilmSync] Error in pause:', e);
    }
  },
  ...
  seek: (seconds) => {
    try {
      ...
      console.error('[FilmSync] Error in seek:', e);
    }
  }
};
