const Logger = {
  info: (...args) => console.log("[FilmSync]", ...args),
  warn: (...args) => console.warn("[FilmSync]", ...args),
  error: (...args) => console.error("[FilmSync]", ...args)
};
...
const PlayerAdapter = {
  ...
  play: () => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        let p = videoElement.play();
        if (p !== undefined && typeof p.catch === 'function') {
          p.catch(e => Logger.error('[FilmSync] Play hatası:', e));
        }
      } catch (e) {
        Logger.error('[FilmSync] Play hatası:', e);
      }
    }
  },
  ...
  pause: () => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        ...
        Logger.error('[FilmSync] Pause hatası:', e);
      }
    }
  },
  ...
  seek: (seconds) => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        ...
        Logger.error('[FilmSync] Seek hatası:', e);
      }
    }
  }
};
