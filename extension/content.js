...
function showAutoJoinOverlay(roomName) {
  ...
  `;
  const roomNameEl = overlay.querySelector('#filmsync-autojoin-roomname');
  if (roomNameEl) {
...
function showNamePromptModal(roomName, callback) {
  ...
  const promptRoomNameEl = modal.querySelector('#filmsync-prompt-roomname');
  if (promptRoomNameEl) {
    promptRoomNameEl.textContent = `"${roomName}" odasına katılacaksınız.`;
  }
