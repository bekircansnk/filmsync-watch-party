...
function showAutoJoinOverlay(roomName) {
  ...
  overlay.innerHTML = `
    ...
    <div id="filmsync-autojoin-roomname-placeholder" style="font-size: 1.2rem; color: #45f3ff; font-weight: 600; margin-bottom: 20px;">
    </div>
  ...
  `;
  const roomNameEl = overlay.querySelector('#filmsync-autojoin-roomname-placeholder');
  if (roomNameEl) {
    roomNameEl.textContent = `"${roomName}" Odasına Katılınıyor...`;
  }
...
function showNamePromptModal(roomName, callback) {
  ...
  modal.innerHTML = `
    <div style="width: 320px; background: rgba(31, 40, 51, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.5); text-align: center; color: #fff;">
      ...
      <div id="filmsync-nameprompt-roomname-placeholder" style="font-size: 0.85rem; color: #66fcf1; margin-bottom: 20px;"></div>
  ...
  `;
  const roomNamePromptEl = modal.querySelector('#filmsync-nameprompt-roomname-placeholder');
  if (roomNamePromptEl) {
    roomNamePromptEl.textContent = `"${roomName}" odasına katılacaksınız.`;
  }
