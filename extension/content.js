...
function showAutoJoinOverlay(roomName) {
  ...
  `;
  const textDiv = overlay.querySelector('#filmsync-autojoin-text');
  if (textDiv) textDiv.textContent = `"${roomName}" Odasına Katılınıyor...`;
...
function showNamePromptModal(roomName, callback) {
  ...
  modal.innerHTML = `
    <div style="width: 320px; background: rgba(31, 40, 51, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.5); text-align: center; color: #fff;">
      ...
      <div id="filmsync-prompt-text" style="font-size: 0.85rem; color: #66fcf1; margin-bottom: 20px;"></div>
  ...
  const promptTextDiv = modal.querySelector('#filmsync-prompt-text');
  if (promptTextDiv) promptTextDiv.textContent = `"${roomName}" odasına katılacaksınız.`;
