const Logger = { info: (...args) => console.log("[FilmSync]", ...args), error: (...args) => console.error("[FilmSync Hata]", ...args), warn: (...args) => console.warn("[FilmSync Uyarı]", ...args) };
    ...
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        ...
        if (playPromise !== undefined) {
          playPromise.catch(e => Logger.error('Oynatma hatası:', e));
        }
      } catch (e) {
        Logger.error('Oynatma başlatılamadı:', e);
      }
    }
  },
  ...
  pause: () => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        ...
        Logger.error('Duraklatma hatası:', e);
      }
    }
  },
