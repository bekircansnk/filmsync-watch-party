const _c = console; const Logger = { info: (...args) => _c.log(...args), error: (...args) => _c.error(...args), warn: (...args) => _c.warn(...args) };
  ...
  play: () => {
    if (PlayerAdapter.isNetflix() || PlayerAdapter.isDisney() || PlayerAdapter.isYouTube()) {
      ...
      try {
        ...
        if (p && p.catch) {
          p.catch(err => Logger.error('[FilmSync] Play promise error:', err));
        }
      } catch (err) {
        Logger.error('[FilmSync] Play exception:', err);
      }
    }
  },
