...
function showAutoJoinOverlay(roomName) {
  ...
  `;
  const textEl = overlay.querySelector('#filmsync-autojoin-text');
  if (textEl) {
...
function showNamePromptModal(roomName, callback) {
  ...
  modal.innerHTML = `
    <div style="width: 320px; background: rgba(31, 40, 51, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.5); text-align: center; color: #fff;">
      ...
      <div id="filmsync-name-prompt-text" style="font-size: 0.85rem; color: #66fcf1; margin-bottom: 20px;"></div>
      
      <div style="text-align: left; margin-bottom: 15px;">
        <label for="promptNameInput" style="font-size: 0.75rem; text-transform: uppercase; color: #45f3ff; font-weight: 600; display: block; margin-bottom: 5px;">Adınız</label>
  ...
  `;
  const textEl = modal.querySelector('#filmsync-name-prompt-text');
  if (textEl) {
    textEl.textContent = `"${roomName}" odasına katılacaksınız.`;
  }
