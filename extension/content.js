...
const PlayerAdapter = {
  ...
  lockEvents: function(duration = 1000) {
    this.isLocked = true;
  ...
  play: function() {
    this.lockEvents(1000);
    if (this.isNetflix() || this.isDisney() || this.isYouTube()) {
      window.postMessage({ source: 'filmsync-content', action: 'play' }, '*');
    } else if (videoElement) {
      const playPromise = videoElement.play();
      if (playPromise !== undefined) {
          playPromise.catch(err => console.error('[FilmSync] Player play hatası:', err));
      }
    }
  },
