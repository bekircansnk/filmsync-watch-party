...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'redirect-tab' && message.url) {
    ...
    if (roomId && username && validIdRegex.test(roomId) && (userId ? validIdRegex.test(userId) : true)) {
    ...
    if (!validIdRegex.test(cleanRoomId) || !validIdRegex.test(hostId)) {
        sendResponse({ status: 'error', error: 'Invalid roomId or hostId format' });
        return true;
    }
    ...
    if (!validIdRegex.test(cleanRoomId) || !validIdRegex.test(userId)) {
        sendResponse({ status: 'error', error: 'Invalid roomId or userId format' });
        return true;
    }
...
function syncCurrentTabMovieUrl(tabId, url) {
  ...
  chrome.storage.local.get(['roomId'], (res) => {
    if (res.roomId && validIdRegex.test(res.roomId)) {
