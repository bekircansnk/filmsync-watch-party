...
const isValidKey = (key) => typeof key === 'string' && /^[a-zA-Z0-9_-]+$/.test(key);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'redirect-tab' && message.url) {
    ...
    if (roomId && username && isValidKey(roomId) && (!userId || isValidKey(userId))) {
    ...
    if (!isValidKey(cleanRoomId) || !isValidKey(hostId)) {
...
function syncCurrentTabMovieUrl(tabId, url) {
  ...
  chrome.storage.local.get(['roomId'], (res) => {
    if (res.roomId && isValidKey(res.roomId)) {
