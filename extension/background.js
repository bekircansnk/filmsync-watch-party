...
const isValidPathParam = (param) => typeof param === 'string' && /^[a-zA-Z0-9_-]+$/.test(param);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'redirect-tab' && message.url) {
    ...
    if (roomId && isValidPathParam(roomId) && (!userId || isValidPathParam(userId)) && username) {
    ...
    if (!isValidPathParam(cleanRoomId) || !isValidPathParam(hostId)) {
      sendResponse({ status: 'error', error: 'Invalid ID format' });
      return true;
    }
    ...
    if (!isValidPathParam(cleanRoomId) || !isValidPathParam(userId)) {
      sendResponse({ status: 'error', error: 'Invalid ID format' });
      return true;
    }
...
function syncCurrentTabMovieUrl(tabId, url) {
  ...
  chrome.storage.local.get(['roomId'], (res) => {
    if (res.roomId && isValidPathParam(res.roomId)) {
