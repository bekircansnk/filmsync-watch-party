...
;// ./src/Teleparty/BrowseScripts/Amazon/amazon_browse_injected.js
...
function addNativePartyButton() {
    ...
    if (parentDiv != null) {
        ...
        if (playButton != null) {
            ...
            if (playButton.className) {
                nativePartyButton.className = playButton.className;
            }
            nativePartyButton.setAttribute("style", "background: linear-gradient(273.58deg, #9E55A0 0%, #EF3E3A 100%); color: #fff; border-radius: 0.375rem; display: flex; align-items: center; justify-content: center; text-align: center; cursor: pointer;");
            ...
            results.push({ button: nativePartyButton, play: (e) => playButton.click() });
        }
    }
    // Home page buttons
    ...
    playTestIdButtons.forEach((btn, idx) => {
        if (btn.parentElement && btn.parentElement.parentElement) {
            ...
            nativePartyButton.setAttribute("style", "width: 100%; background: linear-gradient(273.58deg, #9E55A0 0%, #EF3E3A 100%); color: #fff;");
            nativePartyButton.setAttribute("id", "native-party-button");
            nativePartyButton.innerHTML = "<span>Start a Teleparty</span>";
            ...
            results.push({ button: nativePartyButton, play: (e) => btn.click() });
        }
    });
...
addNativePartyHandler(addNativePartyButton, InternalStreamingServiceName.AMAZON);
