...
;// ./src/Teleparty/BrowseScripts/Disney/disney_browse_injected.js
...
function addNativePartyButton() {
    ...
    const playButton = document.querySelector('a[data-testid="playback-action-button"]');
    if (playButton == null) {
        return undefined;
    }
    ...
    const referenceButtons = parentChildren.slice(0, 2);
    const combinedWidth = referenceButtons.reduce((total, child) => total + child.getBoundingClientRect().width, 0) || 0;
    const targetWidth = (combinedWidth > 0 ? combinedWidth : 245) + 20;
    nativePartyButton.style.width = `${targetWidth}px`;
    nativePartyButton.style.maxWidth = nativePartyButton.style.width;
    nativePartyButton.style.minWidth = nativePartyButton.style.width;
    const parentChildrenCount = parentDiv.children.length;
    if (parentChildrenCount < 3) {
        // If there is no trailer
        playButton.insertAdjacentElement("afterend", nativePartyButton);
    }
    else {
...
addNativePartyHandler(addNativePartyButton, InternalStreamingServiceName.DISNEY_PLUS);
