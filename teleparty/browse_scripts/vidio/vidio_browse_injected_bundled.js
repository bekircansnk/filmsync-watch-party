...
;// ./src/Teleparty/BrowseScripts/Vidio/vidio_browse_injected.js
...
function addNativePartyButton() {
    ...
    containers.forEach((containerSelector, containerIndex) => {
        ...
        mainContainers.forEach((targetContainer, index) => {
            ...
            createdButtons.push({ button: nativePartyButton, play: (e) => playButton.click() });
        });
    });
    return createdButtons.length > 0 ? createdButtons : undefined;
}
addNativePartyHandler(addNativePartyButton, InternalStreamingServiceName.VIDIO);
