// FilmSync Watch Party (Teleparty Clone) 🍿
let roomId = null;
let username = 'Anonim';
let password = '';
let userId = null;
...
function createChatUI() {
  ...
  style.textContent = `
    ...
    #filmsync-mini-toolbar.panel-active {
      right: 342px !important; /* Panel genişliği (330px) + boşluk */
    }
    .filmsync-tool-btn {
      width: 32px;
      height: 32px;
  ...
  root.innerHTML = `
    ...
    <div id="filmsync-mini-toolbar" class="panel-active">
      <button class="filmsync-tool-btn tp-logo" data-tooltip="FilmSync Partisi">
    ...
    <div id="filmsync-chat-panel" class="active">
      <div class="filmsync-header">
        <div class="filmsync-header-top">
          <div class="filmsync-header-title">FilmSync<span>.</span>🍿</div>
...
function showAutoJoinOverlay(roomName) {
  ...
  overlay.innerHTML = `
    ...
    <div style="font-size: 1.2rem; color: #45f3ff; font-weight: 600; margin-bottom: 20px;" id="filmsync-autojoin-text">
    </div>
...
function showNamePromptModal(roomName, callback) {
  ...
  modal.innerHTML = `
    <div style="width: 320px; background: rgba(31, 40, 51, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 18px; padding: 25px; box-shadow: 0 15px 35px rgba(0,0,0,0.5); text-align: center; color: #fff;">
      ...
      <div style="font-size: 0.85rem; color: #66fcf1; margin-bottom: 20px;" id="filmsync-prompt-roomname"></div>
...
// --- 🎬 NETFLIX DETAY KARTLARINA "START A FILMSYNC" BUTONU ENJEKSİYONU ---
function injectNetflixStartButton() {
  ...
  buttonContainers.forEach(container => {
    ...
    startBtn.setAttribute('style', `
      ...
      font-family: inherit;
    `);
    startBtn.innerHTML = '<span>Start a FilmSync</span> 🍿';
...
function setupFullscreenIdleDetector() {
  const handleMouseMove = () => {
    // Sadece tam ekran modundayken gizleme/gösterme yapsın
